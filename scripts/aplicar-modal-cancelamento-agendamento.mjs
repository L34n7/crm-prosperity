...
if (!page.includes("agendaTimeUnitControl")) {
  ...
  page = replaceOnce(
    page,
    cssAnchor,
    `${cssAnchor}${agendaTimeCss}`,
    "estilos das unidades e ações do agendamento"
  );
}
...
if (!automationEnhancer.includes('availability.insertAdjacentElement("afterend", section)')) {
  automationEnhancer = replaceOnce(
    automationEnhancer,
  ...
  automationEnhancer = replaceOnce(
    ...
    "posição final da automação da agenda"
  );

  await writeFile(automationEnhancerPath, automationEnhancer, "utf8");
}

console.log("Configurações, unidades e ações da agenda ajustadas.");
