...
const marker = "CRM_AGENDA_EXPANDABLE_DAY_TIMELINE_V7";
...
if (!source.includes(marker)) {
  ...
  const blockReplacement = `const expandedAvailabilityDays = new Set<string>();
...
function clearOldDecorations`;
  ...
  if (!blockPattern.test(source)) {
    throw new Error(
      "Não foi possível aplicar os cards expansíveis da disponibilidade semanal."
    );
  }
  source = source.replace(blockPattern, blockReplacement);

  const cssEnd = "\n`;\n\nfunction normalize";
  if (!source.includes(cssEnd)) {
    throw new Error("Não foi possível localizar os estilos da agenda.");
  }
...
`;
  ...
  source = source.replace(cssEnd, `${css}${cssEnd}`);
  fs.writeFileSync(file, source, "utf8");
  console.log("Cards expansíveis e linha do tempo da disponibilidade semanal aplicados.");
} else {
  console.log("Cards expansíveis da disponibilidade semanal já aplicados.");
}
