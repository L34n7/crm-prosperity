...
const VARIAVEIS_FIXAS_EMPRESA = new Set([
  "nome_empresa",
  "empresa_nome",
]);
...
type TipoMidiaAutomacao = "image" | "video" | "audio" | "document";
...
function tipoMidiaDoNo(no: AutomacaoNo): TipoMidiaAutomacao | null {
  ...
  if (no.tipo_no === "enviar_arquivo") return "document";
...
function descricaoMidiaEnviada(no: AutomacaoNo) {
  ...
  if (no.tipo_no === "enviar_arquivo") {
    return "Arquivo enviado pela automacao.";
  }
...
export async function executarNo(params: {
  ...
  if (
    ...
    no.tipo_no === "enviar_arquivo"
  ) {
    const midiaUrl = String(no.configuracao_json?.midia_url || "").trim();
    const midiaNome = String(no.configuracao_json?.midia_nome || "").trim();
    ...
    const envioMidia = await enviarMidiaAutomacao({
      ...
      nomeArquivo: tipoMidia === "document" ? midiaNome : "",
      execucaoId,
      noId: no.id,
    });
    ...
    await registrarLog({
      ...
      descricao:
        no.tipo_no === "enviar_imagem"
          ...
          : "Arquivo enviado pela automação.",
      entrada: no.configuracao_json,
      saida: {
        midia_url: midiaUrl,
        legenda,
...
async function obterContatoAutomacao(empresaId: string, execucao: any) {
  ...
  const { data } = await supabaseAdmin
    .from("contatos")
    .select("id, nome, telefone, email, campo_contato, interesse")
    .eq("id", execucao.contato_id)
    .eq("empresa_id", empresaId)
    .maybeSingle();
  ...
  if (!dataForcada && interpretacao.data_invalida_motivo) {
    const mensagemDataInvalida = substituirVariaveisAgenda(
        ...
        "Essa data não é válida ou já passou. Informe uma data futura.\n\nQuando necessário, inclua também o ano.",
      {
        ...
        calendario_nome_novo: agenda?.nome || "",
      }
    );
  ...
  if (interpretacao.data_invalida_motivo) {
    ...
    const mensagemDataInvalida = substituirVariaveisAgenda(
        ...
        "Essa data não é válida ou já passou. Informe uma data futura.\n\nQuando necessário, inclua também o ano.",
      {
        agenda_data_informada: interpretacao.data_informada || "",
        agenda_data_sugestao_ano: interpretacao.data_sugestao_ano || "",
        calendario_nome_novo: agendaConfigurada?.nome || "",
      }
    );
...
async function buscarAgendamentoAutomacao(params: {
  ...
  // CRM_SINGLE_APPOINTMENT_AUTO_SELECTION_V1
...
async function enviarMidiaAutomacao(params: {
  ...
  const { data: mensagemSalva, error: mensagemError } = await supabaseAdmin
    ...
    .insert({
      ...
      conversa_protocolo_id: protocoloAtivo.id,
      remetente_tipo: "bot",
      conteudo: legenda || nomeArquivo || midiaUrl,
      tipo_mensagem:
        tipo === "image"
          ? "imagem"
          : tipo === "video"
