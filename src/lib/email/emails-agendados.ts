...
export async function processarEmailsAgendados(params: {
  agora?: string;
  limite?: number;
} = {}) {
  const agora = params.agora || new Date().toISOString();
  const limite = Math.min(Math.max(Math.floor(params.limite || 25), 1), 100);
  const { data, error } = await supabaseAdmin
  ...
  for (const agendamento of data || []) {
    ...
    try {
      ...
      console.error("[EMAILS AGENDADOS] Erro ao enviar email agendado:", {
