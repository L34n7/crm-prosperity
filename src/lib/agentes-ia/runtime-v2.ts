...
function normalizarEstado(valor: unknown, anterior?: EstadoConversa): EstadoConversa {
  ...
  return {
    ...
    dores: listaCurta(obj.dores, 6).length ? listaCurta(obj.dores, 6) : base.dores,
    interesses: listaCurta(obj.interesses, 6).length ? listaCurta(obj.interesses, 6) : base.interesses,
    ...
    fatos_confirmados: listaCurta(obj.fatos_confirmados, 8).length
      ? listaCurta(obj.fatos_confirmados, 8)
      : base.fatos_confirmados,
  };
}
...
function normalizarSaida(valor: unknown, estadoAnterior: EstadoConversa, fallbackTexto = ""): SaidaAgente {
  ...
  return {
    mensagens,
    estado: normalizarEstado(obj.estado, estadoAnterior),
  };
}
...
function definicoesFerramentas(ativas: Map<TipoFerramenta, Record<string, unknown>>) {
  ...
  if (ativas.has("consultar_conhecimento")) {
    defs.push({
      ...
      description: "Busca fatos na base aprovada. Passe uma consulta curta, preferencialmente de 2 a 8 termos relevantes, sem escrever uma instrução longa.",
      strict: true,
  ...
  if (ativas.has("remarcar_agendamento")) {
    defs.push({
      ...
      parameters: {
        ...
        required: ["agendamento_id", "inicio_at", "fim_at"], additionalProperties: false,
      },
    });
  }
  ...
  if (ativas.has("consultar_contato")) {
    defs.push({
      type: "function", name: "consultar_contato", description: "Consulta os dados do contato atual no CRM.", strict: true,
...
async function validarSlot(params: { empresaId: string; agendaId: string; inicioAt: string; fimAt: string }) {
  ...
  if (Number.isNaN(inicio.getTime()) || Number.isNaN(fim.getTime()) || fim <= inicio) return { ok: false, error: "Período inválido." };
  const data = dataLocalDeIso(inicio.toISOString(), agenda.timezone || "America/Sao_Paulo");
  const resultado = await listarSlotsDisponiveis({ supabase: supabaseAdmin, empresaId: params.empresaId, agendaId: params.agendaId, data, janelaDias: 1, limite: 50 });
  const slot = resultado.slots.find((item: any) => new Date(item.inicio_at).getTime() === inicio.getTime() && new Date(item.fim_at).getTime() === fim.getTime());
...
async function executarFerramenta(nome: TipoFerramenta, args: any, ctx: ContextoExecucao) {
  ...
  if (nome === "consultar_contato") {
    return {
      ok: true,
      contato: ctx.contato ? {
        id: ctx.contato.id, nome: ctx.contato.nome, telefone: ctx.contato.telefone, email: ctx.contato.email,
        empresa: ctx.contato.empresa, origem: ctx.contato.origem, campanha: ctx.contato.campanha,
        status_lead: ctx.contato.status_lead, classificacao: ctx.contato.classificacao,
      } : null,
    };
  }
  ...
  if (nome === "criar_agendamento") {
    ...
    const { data: existente } = await supabaseAdmin.from("agenda_agendamentos")
      ...
      .eq("empresa_id", empresaId).eq("agenda_id", agendaId).eq("conversa_id", conversaId)
      .eq("inicio_at", new Date(inicioAt).toISOString()).eq("fim_at", new Date(fimAt).toISOString())
      .in("status", ["agendado", "confirmado"]).maybeSingle();
    if (existente) { ctx.acaoCriticaExecutada = true; return { ok: true, idempotente: true, agendamento: existente }; }
    const agora = new Date().toISOString();
    const { data: criado, error } = await supabaseAdmin.from("agenda_agendamentos").insert({
      empresa_id: empresaId, agenda_id: agendaId, contato_id: ctx.pendencia.contato_id || null, conversa_id: conversaId,
      titulo: String(args.titulo || "Agendamento").trim() || "Agendamento", nome_cliente: ctx.contato?.nome || null,
      telefone_cliente: ctx.contato?.telefone || ctx.pendencia.numero_destino || null, email_cliente: ctx.contato?.email || null,
      inicio_at: new Date(inicioAt).toISOString(), fim_at: new Date(fimAt).toISOString(), status: "agendado", origem: "api",
      metadata_json: { origem: "agente_ia", agente_id: ctx.agente.id, agente_execucao_id: ctx.execucaoId }, created_at: agora, updated_at: agora,
    }).select("id, titulo, inicio_at, fim_at, status").single();
    if (error || !criado) throw new Error(error?.message || "Erro ao criar agendamento.");
    ctx.acaoCriticaExecutada = true;
    await sincronizarAgendamentoGoogleCalendar({ empresaId, agendaId, agendamentoId: criado.id }).catch((errorSync) => console.error("[AGENTE_IA] Erro ao sincronizar agendamento no Google:", errorSync));
  ...
  if (nome === "remarcar_agendamento") {
    ...
    if (Number.isNaN(inicioAt.getTime()) || Number.isNaN(fimAt.getTime())) return { ok: false, error: "Novo horário inválido." };
...
async function executarFallbackFluxos(pendencia: PendenciaRow) {
  return processAutomationEngineFluxos({
    empresaId: pendencia.empresa_id, conversaId: pendencia.conversa_id, contatoId: pendencia.contato_id || "",
    mensagemTexto: pendencia.conteudo_agregado, numeroDestino: pendencia.numero_destino || "", mensagemId: pendencia.mensagem_ids.at(-1) || null,
  });
}
...
function promptDoAgente(agente: AgenteRow, estado: EstadoConversa, agendas: any[]) {
  ...
  return [
    ...
    agente.tom_voz ? `Características desejadas: ${agente.tom_voz}` : "",
    agente.instrucoes ? `Instruções adicionais: ${agente.instrucoes}` : "",
    "REGRAS OBRIGATÓRIAS:",
    "- Responda em português do Brasil e escreva como uma conversa real de WhatsApp, não como texto institucional.",
    "- A saída final deve conter de 1 a 3 mensagens separadas. Use 1 quando uma frase basta; use 2 ou 3 quando dividir melhora o ritmo da conversa.",
    "- Cada mensagem deve ser curta e ter uma função clara. Não fragmente artificialmente uma frase apenas para criar mais balões.",
...
async function finalizarPendencia(params: {
  pendencia: PendenciaRow; lockToken: string; status?: "processado" | "erro" | "cancelado"; erro?: string | null;
}) {
  ...
  if (resultado.reagendar) {
    const delay = resultado.processar_em ? Math.max(250, new Date(resultado.processar_em).getTime() - Date.now()) : 500;
    void reagendarLocalSeNecessario(params.pendencia.id, delay).catch((erro) => console.error("[AGENTE_IA] Falha ao reagendar nova versão:", erro));
  }
...
export async function processarPendenciaAgenteIa(pendenciaId: string, options: { forcar?: boolean } = {}) {
  ...
  const { data: reservada, error: reservaError } = await supabaseAdmin.rpc("agente_ia_reservar_pendencia", {
    p_pendencia_id: pendenciaId, p_lock_token: lockToken, p_forcar: options.forcar === true,
  });
  ...
  try {
    ...
    const { data: execucao, error: execucaoError } = await supabaseAdmin.from("agente_ia_execucoes").insert({
      ...
      contato_id: pendencia.contato_id || null, mensagem_ids: pendencia.mensagem_ids,
      status: "processando", entrada_resumida: pendencia.conteudo_agregado.slice(0, 4000),
      modelo: agente.modelo || MODELO_PADRAO, started_at: new Date().toISOString(),
    }).select("id").single();
    if (execucaoError || !execucao) throw new Error(execucaoError?.message || "Não foi possível abrir execução do agente.");
    execucaoId = execucao.id;

    ctx = {
      agente: agente as AgenteRow, pendencia, execucaoId, contato, conversa, ferramentasAtivas,
      ferramentasExecutadas: [], acaoCriticaExecutada: false, transferidoHumano: false, respostaEnviada: false,
    };
    ...
    if (saldo.limite !== null && Number(saldo.restantes || 0) <= 0) {
      ...
      await supabaseAdmin.from("agente_ia_execucoes").update({
        status: "fallback", erro: "saldo_tokens_ia_esgotado", finished_at: new Date().toISOString(),
        latencia_ms: Date.now() - inicio, updated_at: new Date().toISOString(),
      }).eq("id", execucaoId);
    ...
    const inputIa: any[] = contexto.historico.length ? contexto.historico : [{ role: "user", content: pendencia.conteudo_agregado }];
    let tokensInput = 0;
    let tokensOutput = 0;
    let tokensTotal = 0;
    ...
    for (let rodada = 0; rodada < MAX_RODADAS_FERRAMENTAS; rodada++) {
      const response: any = await openai.responses.create({
        model: modelo,
        instructions: promptDoAgente(agente as AgenteRow, contexto.estado, contexto.agendas),
        input: inputIa,
        tools,
        parallel_tool_calls: false,
        text: { format: FORMATO_SAIDA },
      } as any);
      ...
      if (!chamadas.length) {
        ...
        try { saidaFinal = normalizarSaida(JSON.parse(texto), contexto.estado); }
        catch { saidaFinal = normalizarSaida(null, contexto.estado, texto); }
        break;
      }
      ...
      for (const chamada of chamadas) {
        ...
        let resultado: any;
        try { resultado = await executarFerramenta(nome, args, ctx); }
        catch (error) { resultado = { ok: false, error: error instanceof Error ? error.message : String(error) }; }
    ...
    if (tokensTotal > 0) {
      await registrarUsoTokensIa({
        empresaId: pendencia.empresa_id, origem: "agente_ia_chat", modelo,
    ...
    if (await temNovaVersao(pendencia)) {
      await supabaseAdmin.from("agente_ia_execucoes").update({
        ...
        ferramentas_json: ctx.ferramentasExecutadas, tokens_input: tokensInput, tokens_output: tokensOutput, tokens_total: tokensTotal,
        latencia_ms: Date.now() - inicio, finished_at: new Date().toISOString(), updated_at: new Date().toISOString(),
    ...
    if (!ctx.transferidoHumano) {
      if (!saidaFinal.mensagens.length) {
        saidaFinal.mensagens = [ctx.acaoCriticaExecutada ? "Pronto, essa etapa foi concluída." : "Me conta um pouco mais para eu te ajudar por aqui."];
      }
    ...
    await supabaseAdmin.from("agente_ia_conversa_estados").upsert({
      empresa_id: pendencia.empresa_id, agente_id: agente.id, conversa_id: pendencia.conversa_id,
      resumo: resumoNovo,
      estado_json: { ...saidaFinal.estado, ultima_acao_critica: ctx.acaoCriticaExecutada, transferido_humano: ctx.transferidoHumano },
      ultima_mensagem_id: pendencia.mensagem_ids.at(-1) || null,
      ultima_interacao_at: new Date().toISOString(), updated_at: new Date().toISOString(),
    }, { onConflict: "agente_id,conversa_id" });

    const finalAgora = new Date().toISOString();
    await supabaseAdmin.from("agente_ia_execucoes").update({
      status: "concluido", resposta: saidaFinal.mensagens.join("\n\n") || null,
      ferramentas_json: ctx.ferramentasExecutadas, tokens_input: tokensInput, tokens_output: tokensOutput, tokens_total: tokensTotal,
      latencia_ms: Date.now() - inicio, finished_at: finalAgora, updated_at: finalAgora,
      metadata_json: { quantidade_mensagens_saida: saidaFinal.mensagens.length, estado_estruturado: saidaFinal.estado },
    }).eq("id", execucaoId);
    await finalizarPendencia({ pendencia, lockToken, status: "processado" });

    return { ok: true, processado: true, agenteId: agente.id, execucaoId, transferidoHumano: ctx.transferidoHumano, mensagens: saidaFinal.mensagens.length };
  } catch (error) {
    ...
    if (podeFallback) {
      try { await executarFallbackFluxos(pendencia); }
      catch (fallbackError) { console.error("[AGENTE_IA] Fallback para fluxos falhou:", fallbackError); }
    } else if (ctx?.acaoCriticaExecutada && !ctx.transferidoHumano && !ctx.respostaEnviada && pendencia.numero_destino && execucaoId) {
      try {
        await enviarMensagemAgente({
          empresaId: pendencia.empresa_id, conversaId: pendencia.conversa_id, agenteId: pendencia.agente_id,
          execucaoId, numeroDestino: pendencia.numero_destino,
    ...
    if (execucaoId) {
      await supabaseAdmin.from("agente_ia_execucoes").update({
        status: podeFallback ? "fallback" : "erro", erro: mensagemErro,
        ferramentas_json: ctx?.ferramentasExecutadas || [], latencia_ms: Date.now() - inicio,
        finished_at: agora, updated_at: agora,
      }).eq("id", execucaoId);
    }
    await finalizarPendencia({ pendencia, lockToken, status: podeFallback ? "processado" : "erro", erro: mensagemErro });
