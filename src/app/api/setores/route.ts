...
export async function GET() {
  try {
    ...
    if (!can(usuario.permissoes, "setores.visualizar")) {
      return NextResponse.json(
        { ok: false, error: "Sem permissao para listar setores" },
        { status: 403 }
      );
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!can(usuario.permissoes, "setores.criar")) {
      return NextResponse.json(
        { ok: false, error: "Sem permissao para criar setores" },
        { status: 403 }
      );
    }
    ...
    if (ativo === false && !can(usuario.permissoes, "setores.alterar_status")) {
      return NextResponse.json(
