...
function validarTamanhoArquivo(file: File) {
  ...
  const limite = tipo === "imagem" ? 5 * 1024 * 1024 : tipo === "audio" || tipo === "video" ? 16 * 1024 * 1024 : 50 * 1024 * 1024;
  if (file.size > limite) {
...
async function limparUploads(paths: string[]) {
  ...
  await supabaseAdmin.storage.from(BUCKET_MENSAGENS_AGENDADAS).remove(paths).catch(() => undefined);
}
...
export async function POST(
  ...
  if (!(await podeEnviarMensagens(usuario))) {
    return NextResponse.json({ ok: false, error: "Sem permissão para enviar mensagens" }, { status: 403 });
  }
  ...
  try {
    ...
    if (!executarEmRaw || !Number.isFinite(executarEm.getTime())) {
      return NextResponse.json({ ok: false, error: "Horário de envio inválido." }, { status: 400 });
    }
    if (executarEm.getTime() <= Date.now() + 15_000) {
      return NextResponse.json({ ok: false, error: "Escolha um horário futuro para o envio." }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ ok: false, error: "Conteúdo do agendamento inválido." }, { status: 400 });
    }
    if (!itensEntrada.length || itensEntrada.length > 20) {
      return NextResponse.json({ ok: false, error: "O agendamento deve ter entre 1 e 20 mensagens." }, { status: 400 });
    }

    const possuiArquivo = itensEntrada.some((item) => item?.tipo === "arquivo");
    if (possuiArquivo && !(await podeEnviarMidia(usuario))) {
      return NextResponse.json({ ok: false, error: "Sem permissão para agendar mídias e arquivos" }, { status: 403 });
    }
    const tamanhoTotal = files.reduce((total, file) => total + file.size, 0);
    if (tamanhoTotal > LIMITE_TOTAL_ARQUIVOS) {
      return NextResponse.json({ ok: false, error: "Os arquivos deste agendamento somam mais de 50 MB." }, { status: 400 });
    }

    const { data: conversa, error: conversaError } = await supabaseAdmin
      .from("conversas")
      .select("id, empresa_id, setor_id, escopo_fila, responsavel_id, status, historico_importado, contato_id, integracao_whatsapp_id")
      .eq("id", conversaId)
    ...
    if (!(await usuarioPodeAcessarConversa(usuario, conversa))) {
      return NextResponse.json({ ok: false, error: "Você não pode agendar mensagens nesta conversa." }, { status: 403 });
    }
    if (conversa.status === "fila" || conversa.responsavel_id !== usuario.id) {
      return NextResponse.json({ ok: false, error: "Assuma a conversa antes de programar mensagens." }, { status: 403 });
    }
    if (isConversaHistoricoImportado(conversa)) {
      return NextResponse.json({ ok: false, error: CONVERSA_HISTORICO_IMPORTADO_MENSAGEM }, { status: 400 });
    }
    if (!conversa.contato_id || !conversa.integracao_whatsapp_id) {
      return NextResponse.json({ ok: false, error: "Conversa sem contato ou integração WhatsApp vinculada." }, { status: 400 });
    }
    ...
    if (!janela.podeEnviarMensagemLivre || !janela.ultimaMensagemRecebidaEm) {
      return NextResponse.json({ ok: false, error: janela.motivoBloqueio || "A janela de atendimento está encerrada.", janela_24h: janela }, { status: 400 });
    }

    const limiteSeguroMs = new Date(janela.ultimaMensagemRecebidaEm).getTime() + LIMITE_SEGURO_JANELA_MS;
    if (!Number.isFinite(limiteSeguroMs) || executarEm.getTime() > limiteSeguroMs) {
      return NextResponse.json(
        {
          ok: false,
          error: "O horário escolhido ultrapassa o limite seguro de 23 horas após a última mensagem do contato.",
          limite_janela_seguranca_em: Number.isFinite(limiteSeguroMs) ? new Date(limiteSeguroMs).toISOString() : null,
        },
        { status: 400 }
      );
    }

    const [{ data: contato, error: contatoError }, { data: protocolo, error: protocoloError }] = await Promise.all([
      supabaseAdmin
        .from("contatos")
    ...
    if (contatoError || !contato?.telefone) {
      return NextResponse.json({ ok: false, error: "Contato sem telefone válido." }, { status: 400 });
    }
    if (protocoloError) throw new Error(protocoloError.message);
    if (!protocolo?.id) {
      return NextResponse.json({ ok: false, error: "A conversa não possui protocolo ativo para este atendimento." }, { status: 400 });
    }
    ...
    for (const [ordem, item] of itensEntrada.entries()) {
      if (item.tipo === "texto") {
        ...
        if (!conteudo) throw new Error(`A mensagem ${ordem + 1} está vazia.`);
      ...
      const storagePath = `${usuario.empresa_id}/${conversaId}/${grupoId}/${String(ordem + 1).padStart(2, "0")}-${randomUUID()}-${nomeSeguro}`;
      ...
      if (uploadError) throw new Error(`Não foi possível armazenar ${file.name}: ${uploadError.message}`);
    ...
    const preview = primeiroTexto?.tipo === "texto"
    ...
    const payload = {
      ...
      assinatura_whatsapp: normalizarAssinaturaWhatsapp(usuario.assinatura_whatsapp),
      itens,
      ...
      template_nome: itens.length > 1 ? `${itens.length} mensagens programadas` : "Mensagem programada",
      tipo_label: itens.length > 1 ? `${itens.length} mensagens da conversa` : "Mensagem da conversa",
      agendamento_titulo: `Mensagem programada · ${contato.nome || contato.telefone}`,
    ...
    if (insertError) throw new Error(`Não foi possível criar o agendamento: ${insertError.message}`);

    return NextResponse.json({
      ok: true,
      message: `Envio programado para ${new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short", timeZone: "America/Sao_Paulo" }).format(executarEm)}.`,
      agendamento: {
        id: agendamento.id,
    ...
    const mensagem = error instanceof Error ? error.message : "Não foi possível programar a mensagem.";
