...
export async function POST(request: Request) {
  ...
  try {
    if (payload.event?.startsWith("subscription.")) {
      const resultado = await sincronizarAssinaturaProsperityPay(payload);
    ...
    if (empresaId && !pagamento.empresa_id) {
      const vinculoPagamento = await supabase
        .from("pagamentos")
        .update({ empresa_id: empresaId, updated_at: new Date().toISOString() })
        .eq("id", pagamento.id);
      if (vinculoPagamento.error) throw vinculoPagamento.error;
    }
