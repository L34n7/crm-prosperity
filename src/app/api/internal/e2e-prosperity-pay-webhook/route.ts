...
export async function GET(request: Request) {
  ...
  if (!secret) {
    return NextResponse.json({ ok: false, error: "PROSPERITY_PAY_WEBHOOK_SECRET ausente na preview." }, { status: 409 });
  }
  ...
  const payload = {
    ...
    order: { id: "7c0afcd3-d204-437d-aefb-147cefbfcdd2" },
    offer: {
  ...
  const response = await fetch("https://crmprosperity.com/api/webhooks/prosperity-pay", {
    method: "POST",
    headers: {
