...
function placeholderPorModo(modo: ModoAssistenteFluxos) {
  if (modo === "adicionar_etapa") {
    return "Ex.: Depois da pergunta sobre procedimento, pergunte o melhor horario e salve em horario_preferido.";
  }
  ...
  return "Ex.: Crie um fluxo para uma clinica de estetica que descubra o procedimento desejado, capture nome e telefone, colete horario preferido e transfira para uma consultora.";
}
...
export default function AssistenteFluxosPanel({
  ...
  async function gerarSugestao() {
    try {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Erro ao gerar sugestao.");
      }
      ...
      setErro(
        error instanceof Error ? error.message : "Erro ao gerar sugestao."
      );
    } finally {
      setGerando(false);
    }
