  ...
  function renderConfigAcao(acao: Acao, index: number) {
    ...
    if (acao.tipo_acao === "whatsapp.enviar_template") {
      ...
      const templates = opcoes.templates.filter(
        (item) => !integracaoId || !item.integracao_whatsapp_id || item.integracao_whatsapp_id === integracaoId,
      );
      return (
        <>
          <label className={styles.formField} style={{ marginTop: 8 }}>
            <span>Integração WhatsApp</span>
            <select value={integracaoId} onChange={(event) => atualizarAcao(index, { configuracao_json: { ...config, integracao_whatsapp_id: event.target.value, template_id: "" } })}>
              <option value="">Selecione</option>
              {opcoes.integracoes_whatsapp.map((item) => <option key={item.id} value={item.id}>{item.nome_conexao}{item.numero ? ` · ${item.numero}` : ""}</option>)}
            </select>
          </label>
          <label className={styles.formField} style={{ marginTop: 8 }}>
            <span>Template</span>
            <select value={String(config.template_id || "")} onChange={(event) => configAcao(index, "template_id", event.target.value)}>
              <option value="">Selecione</option>
