...
.agendaTemplateShell .agendaOverviewHeaderCopy{min-width:0;flex:1}.agendaTemplateShell .agendaOverviewHeaderCopy h2{margin:0;color:var(--crm-text-strong);font-size:19px;font-weight:900}.agendaTemplateShell .agendaOverviewHeaderCopy p{margin:4px 0 0;color:var(--crm-text-muted);font-size:11px}
...
.agendaTemplateShell .agendaOverviewHeroTop{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.agendaTemplateShell .agendaOverviewHero h3{margin:0;color:var(--crm-text-strong);font-size:21px;font-weight:900}.agendaTemplateShell .agendaOverviewHero p{margin:5px 0 0;color:var(--crm-text-muted);font-size:12px}
.agendaTemplateShell .agendaOverviewStatus{min-height:27px;padding:0 10px;border:1px solid var(--crm-primary-border);border-radius:999px;background:var(--crm-primary-soft);color:var(--crm-primary-text);display:inline-flex;align-items:center;font-size:10px;font-weight:900;white-space:nowrap}.agendaTemplateShell .agendaOverviewStatus[data-status="confirmado"],.agendaTemplateShell .agendaOverviewStatus[data-status="realizado"]{border-color:var(--crm-success-border);background:var(--crm-success-bg);color:var(--crm-success-text)}.agendaTemplateShell .agendaOverviewStatus[data-status="cancelado"],.agendaTemplateShell .agendaOverviewStatus[data-status="faltou"]{border-color:var(--crm-danger-border);background:var(--crm-danger-bg);color:var(--crm-danger-text)}
.agendaTemplateShell .agendaOverviewMetrics{margin-top:15px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.agendaTemplateShell .agendaOverviewMetric{min-width:0;padding:11px 12px;border:1px solid var(--crm-border);border-radius:13px;background:color-mix(in srgb,var(--crm-surface) 90%,transparent)}.agendaTemplateShell .agendaOverviewMetric span{display:block;margin-bottom:4px;color:var(--crm-text-muted);font-size:9px;font-weight:850;text-transform:uppercase}.agendaTemplateShell .agendaOverviewMetric strong{display:block;color:var(--crm-text-strong);font-size:12px;font-weight:850;line-height:1.4;overflow-wrap:anywhere}
.agendaTemplateShell .agendaOverviewGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.agendaTemplateShell .agendaOverviewSection{min-width:0;padding:14px;border:1px solid var(--crm-border);border-radius:17px;background:var(--crm-surface);box-shadow:var(--crm-shadow-xs)}.agendaTemplateShell .agendaOverviewSection.full{grid-column:1/-1}.agendaTemplateShell .agendaOverviewSection h4{margin:0 0 11px;color:var(--crm-text-strong);font-size:13px;font-weight:900;display:flex;align-items:center;gap:7px}.agendaTemplateShell .agendaOverviewSection h4:before{content:"";width:8px;height:8px;border-radius:999px;background:var(--crm-primary-strong);box-shadow:0 0 0 4px var(--crm-primary-soft)}
.agendaTemplateShell .agendaOverviewRows{display:grid;gap:9px}.agendaTemplateShell .agendaOverviewRow{display:grid;grid-template-columns:minmax(105px,.7fr) minmax(0,1.3fr);gap:12px;align-items:start}.agendaTemplateShell .agendaOverviewRow span{color:var(--crm-text-muted);font-size:10px;font-weight:750}.agendaTemplateShell .agendaOverviewRow strong{min-width:0;color:var(--crm-text-strong);font-size:11px;font-weight:750;line-height:1.45;overflow-wrap:anywhere}.agendaTemplateShell .agendaOverviewText{margin:0;color:var(--crm-text);font-size:11px;line-height:1.6;white-space:pre-wrap;overflow-wrap:anywhere}
.agendaTemplateShell .agendaOverviewList{display:grid;gap:8px}.agendaTemplateShell .agendaOverviewItem{padding:10px 11px;border:1px solid var(--crm-border);border-radius:12px;background:var(--crm-surface-soft)}.agendaTemplateShell .agendaOverviewItem>b{display:block;margin-bottom:7px;color:var(--crm-text-strong);font-size:11px;font-weight:900}.agendaTemplateShell .agendaOverviewEmpty{padding:8px 0;color:var(--crm-text-muted);font-size:10px}
.agendaTemplateShell .agendaOverviewHistory{position:relative;padding:0 0 12px 18px;border-left:1px solid var(--crm-border-strong)}.agendaTemplateShell .agendaOverviewHistory:last-child{padding-bottom:0}.agendaTemplateShell .agendaOverviewHistory:before{content:"";position:absolute;top:3px;left:-5px;width:9px;height:9px;border-radius:999px;background:var(--crm-success-strong);box-shadow:0 0 0 4px var(--crm-success-bg)}.agendaTemplateShell .agendaOverviewHistory b{display:block;color:var(--crm-text-strong);font-size:11px;text-transform:capitalize}.agendaTemplateShell .agendaOverviewHistory p{margin:3px 0;color:var(--crm-text-muted);font-size:10px;line-height:1.45}.agendaTemplateShell .agendaOverviewHistory small{color:var(--crm-text-soft);font-size:9px}
...
function section(drawer: HTMLElement, title: string) {
  ...
  return Array.from(drawer.querySelectorAll<HTMLElement>(".body > .section")).find(
    (item) => normalize(text(item.querySelector("h3"))).includes(target)
  );
}
...
function fieldValue(container: HTMLElement | undefined, label: string) {
  ...
  const field = Array.from(container.querySelectorAll<HTMLElement>(".field")).find(
    (item) => normalize(text(item.querySelector("label"))) === wanted
  );
  return controlValue(
    field?.querySelector<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(
...
function row(label: string, value: string, raw = false) {
  ...
  return `<div class="agendaOverviewRow"><span>${escapeHtml(label)}</span><strong>${
...
function repeatedItems(container: HTMLElement | undefined, empty: string) {
  ...
  return `<div class="agendaOverviewList">${items
    .map((item, index) => {
      const title = text(item.querySelector(".row b")) || `Item ${index + 1}`;
      const rows = Array.from(item.querySelectorAll<HTMLElement>(".field"))
        .map((field) => {
        ...
        .join("");
      return `<article class="agendaOverviewItem"><b>${escapeHtml(
        title
      )}</b><div class="agendaOverviewRows">${rows}</div></article>`;
    })
    .join("")}</div>`;
}

function createOverview(source: HTMLElement) {
  ...
  const related =
    section(source, "Registros relacionados") ||
    section(source, "Registro relacionado") ||
    section(source, "Imóvel relacionado") ||
    section(source, "Procedimento relacionado");
  ...
  const historyHtml = Array.from(history?.querySelectorAll<HTMLElement>(".hist") || [])
    .map(
      (item) => `<article class="agendaOverviewHistory"><b>${escapeHtml(
        text(item.querySelector("b")).replaceAll("_", " ") || "Alteração"
      )}</b><p>${escapeHtml(
        text(item.querySelector("p")) || "Alteração registrada."
      )}</p><small>${escapeHtml(text(item.querySelector("small")))}</small></article>`
    )
    .join("");
  ...
  overview.innerHTML = `
    ...
    <div class="agendaOverviewBody">
      <section class="agendaOverviewHero">
        <div class="agendaOverviewHeroTop"><div><h3>${escapeHtml(title)}</h3><p>${escapeHtml(
          type
        )}</p></div><span class="agendaOverviewStatus" data-status="${escapeHtml(
    statusKey
  )}">${escapeHtml(status || "Sem status")}</span></div>
        <div class="agendaOverviewMetrics">
          <div class="agendaOverviewMetric"><span>Início</span><strong>${escapeHtml(
            dateTime(start)
          )}</strong></div>
          <div class="agendaOverviewMetric"><span>Duração</span><strong>${escapeHtml(
            duration(start, end)
          )}</strong></div>
          <div class="agendaOverviewMetric"><span>Cliente</span><strong>${escapeHtml(
            clientName || "Cliente não informado"
          )}</strong></div>
          <div class="agendaOverviewMetric"><span>Responsável</span><strong>${escapeHtml(
            responsible
          )}</strong></div>
        </div>
      </section>
      <div class="agendaOverviewGrid">
        <section class="agendaOverviewSection"><h4>Informações principais</h4><div class="agendaOverviewRows">
          ${row("Término", dateTime(end))}${row("Prioridade", priority)}${row(
    "Confirmação",
    confirmation || "Não informada"
  )}${row("Local", location || "Não informado")}${
    meeting
      ? row(
          "Reunião",
          `<a class="agendaOverviewLink" href="${escapeHtml(
            meeting
          )}" target="_blank" rel="noopener noreferrer">Abrir link da reunião</a>`,
          true
        )
      : row("Reunião", "Não informada")
  }
        </div></section>
        <section class="agendaOverviewSection"><h4>Cliente</h4><div class="agendaOverviewRows">${row(
          "Nome",
          clientName || "Não informado"
        )}${row("Telefone", clientPhone || "Não informado")}${row(
    "E-mail",
    clientEmail || "Não informado"
  )}</div></section>
        <section class="agendaOverviewSection full"><h4>Descrição</h4><p class="agendaOverviewText">${escapeHtml(
          description || "Nenhuma descrição foi adicionada."
        )}</p></section>
        <section class="agendaOverviewSection"><h4>Participantes</h4>${repeatedItems(
          participants,
          "Nenhum participante adicional."
        )}</section>
        <section class="agendaOverviewSection"><h4>Registros relacionados</h4>${repeatedItems(
          related,
          "Nenhum registro relacionado."
        )}</section>
        <section class="agendaOverviewSection full"><h4>Lembretes</h4>${repeatedItems(
          reminders,
          "Nenhum lembrete ativo."
        )}</section>
        <section class="agendaOverviewSection full"><h4>Resultado e informações internas</h4><div class="agendaOverviewRows">${row(
          "Status final",
          finalStatus || status || "Não informado"
        )}${row("Resultado", resultSummary || "Não informado")}</div><p class="agendaOverviewText" style="margin-top:11px">${escapeHtml(
          internalNotes || "Nenhuma observação interna."
        )}</p></section>
        <section class="agendaOverviewSection full"><h4>Histórico</h4>${
          historyHtml || '<div class="agendaOverviewEmpty">Sem alterações registradas.</div>'
        }</section>
      </div>
    </div>
  ...
  overview.querySelectorAll<HTMLButtonElement>('[data-action="close"]').forEach((button) => {
...
function AgendaOverviewEnhancer() {
  useEffect(() => {
    ...
    const apply = () => {
      ...
      if (disposed) return;
      shell
        .querySelectorAll<HTMLElement>(".a2 .overlay > .drawer:not(.agendaOverviewDrawer)")
        .forEach((drawer) => {
          if (drawer.dataset.agendaOverviewEditMode !== "true") createOverview(drawer);
        });
    };

    const schedule = () => {
      if (!disposed && !frame) frame = window.requestAnimationFrame(apply);
    };
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        shell
          .querySelector<HTMLButtonElement>('.agendaOverviewDrawer [data-action="close"]')
          ?.click();
      }
    };
