...
export default function CatalogPropertyModal({ catalogoId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/imoveis/catalogo?imovel=${encodeURIComponent(catalogoId)}&limite=1`, {
      cache: "no-store",
    ...
    <>
            ...
            <div className={styles.catalogDetailScroll}>
              <div
                className={`${styles.catalogAlbum} ${
                  fotos.length <= 1 ? styles.catalogAlbumSingle : ""
                }`}
                ...
                <div className={styles.catalogDetailContent}>
                  ...
                  <section className={styles.catalogDetailSection}>
                    ...
                    <dl className={styles.catalogValueList}>
                      <div>
                        <dt>Condomínio</dt>
                        <dd>{formatarMoedaDetalhada(imovel.valor_condominio)}</dd>
                      </div>
                      <div>
                        <dt>IPTU</dt>
                  ...
                  <section className={styles.catalogDates}>
                    ...
                    <div>
                      <span>
                        Criado em <strong>{formatarDataImovel(imovel.created_at)}</strong>
                      </span>
                      <span>
                        Atualizado em <strong>{formatarDataImovel(imovel.updated_at)}</strong>
                      </span>
                    </div>
                  </section>
                </div>
      ...
      {imovel && galeriaAberta && fotos[fotoAtiva] ? (
        <div
          ...
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setGaleriaAberta(false);
          }}
        >
          <button
          ...
          >
            <img
              src={fotos[fotoAtiva]}
              alt={`${tituloDoImovel(imovel)} — foto ${fotoAtiva + 1}`}
              referrerPolicy="no-referrer"
            />
          </div>
