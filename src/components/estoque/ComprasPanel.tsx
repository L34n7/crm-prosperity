...
type Pedido = {
  ...
  conta_pagar_id: string | null; conta_pagar_status: string | null;
      ...
      {pedidosFiltrados.map((pedido) => {
        ...
        return <article className={styles.order} key={pedido.id}>
          ...
          <div className={styles.orderActions}>
            ...
            {pedido.status !== "cancelado" && pedido.conta_pagar_id && Number(pedido.valor_pago) < Number(pedido.total) && podeFinanceiro ? <button onClick={() => { setPedidoSelecionado(pedido); setPagamentoForm({ valor: String(Math.max(0, Number(pedido.total) - Number(pedido.valor_pago))), forma: "pix", vencimento_em: hoje(), referencia: "", observacao: "" }); setModal("pagamento"); }}><Banknote size={15} /> Pagamento</button> : null}
          </div>
        </article>;
      })}
