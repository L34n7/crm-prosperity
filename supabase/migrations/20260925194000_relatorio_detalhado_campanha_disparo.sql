drop function if exists public.relatorio_whatsapp_disparo_campanha(uuid, uuid);

create function public.relatorio_whatsapp_disparo_campanha(
  p_empresa_id uuid,
  p_campanha_id uuid
)
returns table (
...
as $$
  with itens as (
    select
      ...
      m.metadata_json as mensagem_metadata_json,
      m.updated_at as mensagem_updated_at,
  ...
  select
    ...
    case
      ...
      when coalesce(i.status_mensagem, '') = 'lida'
        then 'Lido'
      when coalesce(i.status_mensagem, '') = 'entregue'
        then 'Entregue'
      when coalesce(i.status_mensagem, '') = 'enviada'
        then 'Enviado'
      else 'Pendente'
    end as situacao,
    ...
    i.enviado_em,
    case
      when coalesce(i.status_mensagem, '') = 'lida' then
        coalesce(
          case
            when coalesce(
              i.mensagem_metadata_json #>> '{whatsapp_status,timestamp_evento_whatsapp}',
              ''
            ) ~ '^[0-9]+([.][0-9]+)?$'
            then to_timestamp(
              (i.mensagem_metadata_json #>> '{whatsapp_status,timestamp_evento_whatsapp}')::double precision
            )
            else null
          end,
          case
            when coalesce(
              i.mensagem_metadata_json #>> '{whatsapp_status,raw_status,timestamp}',
              ''
            ) ~ '^[0-9]+([.][0-9]+)?$'
            then to_timestamp(
              (i.mensagem_metadata_json #>> '{whatsapp_status,raw_status,timestamp}')::double precision
            )
            else null
          end,
          i.mensagem_updated_at
        )
      else null
    end as lido_em,
...
comment on function public.relatorio_whatsapp_disparo_campanha(uuid, uuid) is
  'Retorna o relatório detalhado de uma campanha com status final, primeira resposta e horários de envio, leitura e resposta.';
