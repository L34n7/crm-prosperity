-- Consolida a leitura operacional por contato e numero do WhatsApp.
-- Integracoes reconectadas recebem um novo UUID, mas preservam o mesmo
-- phone_number_id da Meta. Historicos importados e conversas de conexoes
-- anteriores permanecem no banco, porem nao devem aparecer como conversas
-- paralelas quando ja existe uma conversa canonica do mesmo contato/numero.

CREATE OR REPLACE VIEW public.conversas_canonicas
WITH (security_invoker = true)
AS
WITH identificadas AS (
  SELECT
    c,
    CASE
      WHEN c.canal = 'whatsapp' THEN COALESCE(
        integracao.phone_number_id,
        c.integracao_whatsapp_phone_number_id_anterior,
        'integracao:' || c.integracao_whatsapp_id::text,
        'conversa:' || c.id::text
      )
      ELSE 'conversa:' || c.id::text
    END AS chave_integracao_logica,
    CASE
      WHEN c.status IN (
        'aberta',
        'bot',
      ...
      WHEN c.origem_atendimento = 'historico_coexistence' THEN 2
      ELSE 1
    END AS prioridade_canonica
  FROM public.conversas c
  LEFT JOIN public.integracoes_whatsapp integracao
    ON integracao.id = c.integracao_whatsapp_id
   AND integracao.empresa_id = c.empresa_id
),
ranqueadas AS (
  SELECT
    c,
...
REVOKE ALL ON public.conversas_canonicas
  FROM public, anon, authenticated, service_role;
GRANT SELECT ON public.conversas_canonicas TO service_role;
...
AS $function$
  ...
  FROM public.conversas_canonicas c
