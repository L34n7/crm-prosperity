-- Corrige a data de atualizacao e as chaves usadas nas ordenacoes do catalogo.
-- Areas zeradas passam a usar os campos alternativos ou a metragem presente
-- no texto; titulos ignoram formatacao inicial na ordenacao alfabetica.

create or replace view public.catalogo_imoveis_global
...
select
  ...
  imovel.vagas,
  coalesce(
    nullif(imovel.area_m2, 0),
    nullif(
      replace(
        substring(
          concat_ws(' ', imovel.titulo, imovel.descricao)
          from '([0-9]{1,6}(?:[.,][0-9]{1,2})?)[[:space:]]*(?:m²|m2|metros?[[:space:]]+quadrados?)'
        ),
        ',',
        '.'
      ),
...
select
  ...
  externo.tipo,
  externo.finalidade,
  lower(coalesce(nullif(trim(externo.status_origem), ''), externo.status)) as status,
  externo.valor,
  ...
  externo.vagas,
  coalesce(
    nullif(externo.area_m2, 0),
    nullif(externo.area_util_m2, 0),
    nullif(externo.area_total_m2, 0),
    nullif(externo.area_terreno_m2, 0),
    nullif(
      replace(
        substring(
          concat_ws(' ', externo.titulo, externo.descricao)
          from '([0-9]{1,6}(?:[.,][0-9]{1,2})?)[[:space:]]*(?:m²|m2|metros?[[:space:]]+quadrados?)'
        ),
        ',',
        '.'
  ...
  externo.created_at,
  coalesce(externo.atualizado_origem_em, externo.updated_at) as updated_at,
  regexp_replace(
    translate(
      lower(trim(externo.titulo)),
      'áàâãäåéèêëíìîïóòôõöúùûüçñ',
      'aaaaaaeeeeiiiiooooouuuucn'
    ),
    '^[^a-z0-9]+',
...
where externo.status <> 'arquivado'
  or lower(trim(coalesce(externo.status_origem, ''))) = 'indisponivel';
