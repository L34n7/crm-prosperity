-- Mantém a permissão de atribuir/reatribuir para conversas operacionais do setor,
-- sem ampliar a listagem para atendimentos encerrados de outros responsáveis.
-- A mesma regra é aplicada à lista, aos contadores e às conversas não lidas.

-- contar_conversas_nao_lidas(p_empresa_id uuid, p_usuario_id uuid, p_is_admin boolean, p_setores_ids uuid[], p_usuario_pode_atribuir boolean, p_status text, p_prioridade text, p_contato_id uuid, p_setor_id uuid, p_responsavel_id uuid, p_busca text, p_canal text, p_lista_id uuid)
CREATE OR REPLACE FUNCTION public.contar_conversas_nao_lidas(p_empresa_id uuid, p_usuario_id uuid, p_is_admin boolean, p_setores_ids uuid[] DEFAULT '{}'::uuid[], p_usuario_pode_atribuir boolean DEFAULT false, p_status text DEFAULT NULL::text, p_prioridade text DEFAULT NULL::text, p_contato_id uuid DEFAULT NULL::uuid, p_setor_id uuid DEFAULT NULL::uuid, p_responsavel_id uuid DEFAULT NULL::uuid, p_busca text DEFAULT NULL::text, p_canal text DEFAULT NULL::text, p_lista_id uuid DEFAULT NULL::uuid)
 RETURNS integer
 LANGUAGE sql
 STABLE
...
CREATE OR REPLACE FUNCTION public.listar_conversas_resumo(p_empresa_id uuid, p_usuario_id uuid, p_is_admin boolean, p_setores_ids uuid[], p_usuario_pode_atribuir boolean, p_status text, p_prioridade text, p_contato_id uuid, p_setor_id uuid, p_responsavel_id uuid, p_busca text, p_canal text, p_chip text, p_lista_id uuid, p_cursor_last_message_at timestamp with time zone, p_cursor_created_at timestamp with time zone, p_cursor_id uuid, p_limite integer)
 RETURNS SETOF jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public', 'pg_temp'
...
-- obter_contadores_conversas(p_empresa_id uuid, p_usuario_id uuid, p_is_admin boolean, p_setores_ids uuid[], p_usuario_pode_atribuir boolean, p_status text, p_prioridade text, p_contato_id uuid, p_setor_id uuid, p_responsavel_id uuid, p_busca text, p_canal text, p_lista_id uuid)
CREATE OR REPLACE FUNCTION public.obter_contadores_conversas(p_empresa_id uuid, p_usuario_id uuid, p_is_admin boolean, p_setores_ids uuid[], p_usuario_pode_atribuir boolean, p_status text, p_prioridade text, p_contato_id uuid, p_setor_id uuid, p_responsavel_id uuid, p_busca text, p_canal text, p_lista_id uuid)
